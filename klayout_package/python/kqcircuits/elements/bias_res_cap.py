...
@add_parameters_from(BiasResonator)
...
class BiasResCap(Element):
    ...
    cap_distance = Param(pdt.TypeDouble, "Capacitor distance from bottom of ground gap", 20, unit="μm")
    ...
    def build(self):
        ...
        self.insert_cell(
            BiasResonator, pya.DPoint(0, 0), f"BR",
            bias_width=self.a,
        ...
        cap_center_x = self.refpoints[f"BR_inductor_ground"].x
        cap_center_y = self.refpoints[f"BR_inductor_ground"].y - self.cap_distance
        ...
        self.insert_cell(
                WaveguideComposite, 
                nodes=[
                Node(self.refpoints[f"BR_inductor_ground"]),
    ...
    def get_sim_ports(cls, simulation):
        ...
        # Get ports from BiasRes (junction lumped model if enabled)
        ports.extend(BiasResonator.get_sim_ports(simulation))
