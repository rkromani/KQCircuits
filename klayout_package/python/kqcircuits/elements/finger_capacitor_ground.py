...
class FingerCapacitorGround(Element):
    """The PCell declaration for a grounded finger capacitor.
    ...
    ground_padding = Param(pdt.TypeDouble, "Ground plane padding", 10, unit="μm")
    ...
    def build(self):
        ...
        #x_mid = self.finger_area_length() / 2 #+ self.finger_gap_end + self.a/2
        x_mid = self.finger_gap_end + self.finger_length/2
        ...
        region_ground = pya.Region(
            pya.DPolygon(
                [
                    pya.DPoint(-x_left, -y_mid-self.ground_padding),
                    pya.DPoint(-x_left, y_mid+self.ground_padding),
                    pya.DPoint(x_right, y_mid+self.ground_padding),
                    pya.DPoint(x_right, -y_mid-self.ground_padding),
                ]
        ...
        region_ground_right = pya.Region(
            pya.DPolygon(
                [
                    ...
                    pya.DPoint(x_right, y_mid + self.ground_padding),
                    pya.DPoint(x_right + self.ground_padding, y_mid + self.ground_padding),
                    pya.DPoint(x_right + self.ground_padding, -y_mid - self.ground_padding),
                    pya.DPoint(x_right, -y_mid - self.ground_padding),
                    pya.DPoint(x_right, -y_mid),
                    pya.DPoint(x_mid, -y_mid),
        ...
        region_ground_left = pya.Region(
            pya.DPolygon(
                [
                    ...
                    pya.DPoint(-x_left, y_mid + self.ground_padding),
                    pya.DPoint(-x_left - self.ground_padding, y_mid + self.ground_padding),
                    pya.DPoint(-x_left - self.ground_padding, -y_mid - self.ground_padding),
                    pya.DPoint(-x_left, -y_mid - self.ground_padding),
                    pya.DPoint(-x_left, -y_mid),
                    pya.DPoint(-x_mid, -y_mid),
        ...
        for i in range(self.finger_number):
            ...
            if ((i % 2) == 0):
                polys_fingers.append(
                                    pya.DPolygon(
                        [
                            pya.DPoint(self.finger_length/2, y + self.finger_width),
                            pya.DPoint(self.finger_length/2, y),
                            pya.DPoint(-self.finger_length/2, y),
                            pya.DPoint(-self.finger_length/2, y + self.finger_width),
                        ]
                    )
                ...
                polys_fingers.append(
                    pya.DPolygon(
                        [
                            pya.DPoint(self.finger_gap_end + self.finger_length/2 + self.a/2, y + self.finger_width),
                            pya.DPoint(self.finger_gap_end + self.finger_length/2 + self.a/2, y),
                            pya.DPoint(self.finger_gap_end + self.a/2, y),
                            pya.DPoint(self.finger_gap_end + self.a/2, y + self.finger_width),
                        ]
                    ) 
                )
                polys_fingers.append(
        ...
        #region_etch = region_fingers
        region_etch = region_ground_left + region_ground_right + region_fingers + region_center
        ...
        self.add_port("a", pya.DPoint(0, -y_mid), pya.DVector(0, -1))
        self.add_port("b", pya.DPoint(0, y_mid), pya.DVector(0, 1))
    ...
    def add_waveguides(self, region, x_end, y_left, y_right):
        y_tot_height = self.finger_area_width() + self.ground_padding
        x_guide_l = self.fixed_length / 2
        x_guide_r = self.fixed_length / 2 
        if self.fixed_length != 0 and x_guide_l < x_end:
            ...
            region += pya.Region(
                pya.DPolygon(
                    [
                        ...
                        pya.DPoint(-x_guide_l, -y_left),
                        pya.DPoint(-x_guid_l, y_left),
                        pya.DPoint(-x_end, y_left),
                    ]
                ).to_itype(self.layout.dbu)
            ) + pya.Region(
                pya.DPolygon(
                    [
                        ...
                        pya.DPoint(x_guide_l, -y_right),
                        pya.DPoint(x_guid_l, y_right),
                        pya.DPoint(x_end, y_right),
                    ]
                ).to_itype(self.layout.dbu)
            )
