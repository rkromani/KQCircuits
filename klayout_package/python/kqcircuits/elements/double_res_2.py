...
class DoubleRes2(Element):
    ...
    c_gap = Param(pdt.TypeDouble, "Capacitor gap", 2, unit="μm")
    c_width = Param(pdt.TypeDouble, "Capacitor pad width", 20, unit="μm")
    c_length = Param(pdt.TypeDouble, "Capacitor pad length", 200, unit="μm")
    c_ground_spacing = Param(pdt.TypeDouble, "Spacing from capacitor pad to ground", 50, unit="μm")
    ...
    def build(self):
        ...
        self.ground_gap_bottom_cap = self.ground_gap_bottom - self.c_ground_spacing - self.c_length
        self.ground_gap_width_cap = self.l_connection_spacing + self.c_width + 2 * self.c_gap 

        ground_cutout_pts = [
            pya.DPoint(self.ground_gap_width_cap/2, self.ground_gap_bottom),
            pya.DPoint(self.ground_gap_width_cap/2, self.ground_gap_bottom_cap),
            pya.DPoint(-self.ground_gap_width_cap/2, self.ground_gap_bottom_cap),
            pya.DPoint(-self.ground_gap_width_cap/2, self.ground_gap_bottom)
        ]
        ...
        cap_pts = [
            pya.DPoint(self.l_connection_spacing/2 + self.c_width/2, self.ground_gap_bottom),
            pya.DPoint(self.l_connection_spacing/2 + self.c_width/2, self.ground_gap_bottom - self.c_length),
            pya.DPoint(self.l_connection_spacing/2 - self.c_width/2, self.ground_gap_bottom - self.c_length),
            pya.DPoint(self.l_connection_spacing/2 - self.c_width/2, self.ground_gap_bottom),
        ]
        cap_polygon = pya.DPolygon(cap_pts)
        cap_region = pya.Region(cap_polygon.to_itype(self.layout.dbu))
        cap_region_mirrored = cap_region.transformed(pya.Trans(pya.Trans.M90))

        bias_pts = [
            pya.DPoint(self.l_connection_spacing/2 - self.c_width/2 - self.c_gap, self.ground_gap_bottom),
            pya.DPoint(self.l_connection_spacing/2 - self.c_width/2 - self.c_gap, self.ground_gap_bottom - self.c_length),
            pya.DPoint(self.a/2, self.ground_gap_bottom - self.c_length),
            pya.DPoint(self.a/2, self.ground_gap_bottom_cap),
            pya.DPoint(-self.a/2, self.ground_gap_bottom_cap),
            pya.DPoint(-self.a/2, self.ground_gap_bottom - self.c_length),
            pya.DPoint(-self.l_connection_spacing/2 + self.c_width/2 + self.c_gap, self.ground_gap_bottom - self.c_length),
            pya.DPoint(-self.l_connection_spacing/2 + self.c_width/2 + self.c_gap, self.ground_gap_bottom),
        ]
        bias_polygon = pya.DPolygon(bias_pts)
        bias_region = pya.Region(bias_polygon.to_itype(self.layout.dbu))

        bias_line_end_y = self.ground_gap_bottom_cap - self.bias_line_length
        pts_bias_line_left = [
            pya.DPoint(-self.a/2, self.ground_gap_bottom_cap),
            pya.DPoint(-self.a/2 - self.b, self.ground_gap_bottom_cap), 
            pya.DPoint(-self.a/2 - self.b, bias_line_end_y), 
            pya.DPoint(-self.a/2, bias_line_end_y), 
        ]
        bias_line_left_region = pya.Region(pya.DPolygon(pts_bias_line_left).to_itype(self.layout.dbu))
        pts_bias_line_right = [
            pya.DPoint(self.a/2, self.ground_gap_bottom_cap),
            pya.DPoint(self.a/2 + self.b, self.ground_gap_bottom_cap), 
            pya.DPoint(self.a/2 + self.b, bias_line_end_y), 
        ...
        bias_line_right_region = pya.Region(pya.DPolygon(pts_bias_line_right).to_itype(self.layout.dbu))

        self.cell.shapes(self.get_layer("base_metal_gap_wo_grid")).insert(
            ground_cutout - cap_region - cap_region_mirrored - bias_region + bias_line_left_region + bias_line_right_region
        )
