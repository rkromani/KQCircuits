...
class RKRBridge2(Junction):
    ...
    palm_width = Param(pdt.TypeDouble, "Width of palm region", 1, unit="μm")
    ...
    def get_ebeam_shape(self):
        ...
        finger_left = -self.bridge_length/2
        finger_right = self.bridge_length/2
        finger_top = -self.bridge_width/2
        finger_taper_left = -self.taper_base/2
        finger_taper_right = self.taper_base/2
        ...
        palm_left = -self.palm_width/2
        palm_right = self.palm_width/2
        palm_bottom = self.bridge_width/2
        palm_taper_bottom = self.pad_offset - self.taper_height
        palm_taper_left = -self.taper_base/2
        palm_taper_right = self.taper_base/2
        palm_pts = [
            pya.DPoint(palm_taper_left, self.pad_offset),
            pya.DPoint(palm_left, palm_taper_bottom),
            pya.DPoint(palm_left, palm_bottom),
            pya.DPoint(palm_right, palm_bottom),
            pya.DPoint(palm_right, palm_taper_bottom),
            pya.DPoint(palm_taper_right, self.pad_offset),
        ]
        palm_region = pya.Region(pya.DPolygon(palm_pts).to_itype(self.layout.dbu))

        ebeam_region = pads_region + finger_region + palm_region
        ebeam_region.merge()
        return ebeam_region
