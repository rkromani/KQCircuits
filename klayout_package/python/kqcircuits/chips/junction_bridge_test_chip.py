...
class JunctionBridgeTestChip(Chip):
    ...
    rotate_junctions_180 = Param(pdt.TypeBoolean, "Whether or not to rotate junctions 180 degrees", False)
    ...
    def build(self):
        ...
        taper_width = 10
        palm_width = 7
        bridge_lengths = np.asarray([0.1, 0.25, 0.5, 1, 2, 3, 4, 5, 6])
        bridge_widths = np.asarray([0.1, 0.15, 0.25, 0.5])
        #bridge_lengths = np.asarray([0.1, 0.3, 1, 3])
        #bridge_widths = np.asarray([0.1, 0.15])


        chip_center_x = 7500/2
        chip_center_y = 7500/2
        n_junction_x_group = 5
        n_junction_x = n_junction_x_group * len(bridge_widths)
        n_junction_y = len(bridge_lengths)
        spacing_x = 250
        spacing_x_big = 1400
        spacing_y = 500
        start_x = chip_center_x - (n_junction_x_group - 1)/2 * spacing_x - (len(bridge_widths)-1)/2 * spacing_x_big
        ...
        pos_x = np.array([
                          ...
                          for j in range(len(bridge_widths))
                          ])
        pos_x = np.sort(pos_x)
        ...
        while i < n_junction_x:
            ...
            while j < n_junction_y:
                ...
                if (j == len(bridge_lengths) - 1) and (i > n_junction_x - 11) and (i < n_junction_x - 5):
                    if self.rotate_junctions_180:
                        coord = pya.DTrans(2, False, pos_x[i], pos_y[j])
                    else:
                        coord = pya.DTrans(0, False, pos_x[i], pos_y[j])
    
                    self.insert_cell(
                        RKRBridge2, coord, f"JJ_{i}_{j}", 
                ...
                elif (j == len(bridge_lengths) - 1) and (i > n_junction_x - 6):
                    if self.rotate_junctions_180:
                        coord = pya.DTrans(2, False, pos_x[i], pos_y[j])
                    else:
                        coord = pya.DTrans(0, False, pos_x[i], pos_y[j])
                    ...
                    self.insert_cell(
                        ...
                        palm_width=palm_width, taper_base=taper_width, 
                        bridge_length=bridge_lengths[j],
                        bridge_width=bridge_widths[int(i/5)]
                        #**element_params[i],
                    )
                    ...
                    if i % (n_junction_x_group) == 0:
                        produce_label(
                            self.cell,
                            label=str(bridge_lengths[j]) + " x " + str(bridge_widths[int(i/5)]),
                            location=pya.DPoint(pos_x[i], pos_y[j] - spacing_y/2),
                            origin=LabelOrigin.TOPLEFT,
                            origin_offset=0,
                            margin=10,
