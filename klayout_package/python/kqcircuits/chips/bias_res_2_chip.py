...
class BiasRes2Chip(Chip):
    ...
    def build(self):
        ...
        res_params = [{"l_coupling_distance": 20, "l_coupling_length": 40},
                      {"l_coupling_distance": 20, "l_coupling_length": 300}, 
                      {"l_coupling_distance": 20, "l_coupling_length": 600, "ground_cutout_width": 850}, 
                      {"l_coupling_distance": 20, "l_coupling_length": 1200, "ground_cutout_width": 1100},]
        ...
        while i < n_res:
            if i < n_res/2:
                ...
                BR_coord = pya.DTrans(0, False, center_x - (i - (n_res-1)/2)*spacing + 300, center_y)
            BR_coords.append(BR_coord)

            self.insert_cell(
            BiasResonator2, BR_coord, f"BR{i}", 
        ...
        while i < n_res:
            ...
            if i < n_res/2:
                rot = 2
                cap_center_y = self.refpoints[f"BR{i}_inductor_ground"].y + self.cap_y_distance
                cap_center_x = self.refpoints[f"BR{i}_inductor_ground"].x - self.cap_x_distance
            else:
                rot = 0
                cap_center_y = self.refpoints[f"BR{i}_inductor_ground"].y - self.cap_y_distance
                cap_center_x = self.refpoints[f"BR{i}_inductor_ground"].x + self.cap_x_distance
            ...
            if i != cap_to_skip:
                self.insert_cell(
                    FingerCapacitorGroundV3, pya.DTrans(rot+1, False, self.refpoints[f"BR{i}_inductor_ground"].x,
                                                         self.refpoints[f"BR{i}_inductor_ground"].y), f"CAP{i}a",
                    a=self.a, b=self.b, finger_number=7, 
                )

                self.insert_cell(
                    FingerCapacitorGroundV3, pya.DTrans(rot, False, cap_center_x, cap_center_y), f"CAP{i}b",
                    a=self.a, b=self.b,
                )
                ...
                self.insert_cell(
                        WaveguideComposite, 
                        nodes=[
                            Node(self.refpoints[f"CAP{i}a_bottom_port"]),
                            Node((self.refpoints[f"CAP{i}b_top_port"].x, self.refpoints[f"BR{i}_inductor_ground"].y)),
                            Node(self.refpoints[f"CAP{i}b_top_port"])
                        ],
                    )
        ...
        splitter_inst, splitter_ref = self.insert_cell(
            ...
            pya.DTrans(self.refpoints[f"CAP{i}b_bottom_port"].x, 7500 - self.bias_rail_y),
            f"T{i}",
            **t_cross_parameters(
                a=self.a,
                b=self.b,
        ...
        self.insert_cell( WaveguideComposite, nodes=[
                    ...
                    Node(self.refpoints[f"CAP0b_bottom_port"])])
        self.insert_cell( WaveguideComposite, nodes=[
                    Node(self.refpoints[f"T0_port_left"]),
                    Node((self.refpoints[f"CAP1b_bottom_port"].x, self.refpoints[f"T0_port_left"].y)),
                    Node(self.refpoints[f"CAP1b_bottom_port"])])
        ...
        splitter_inst, splitter_ref = self.insert_cell(
            WaveguideCoplanarSplitter,
            pya.DTrans(2, False, self.refpoints[f"CAP{i}b_bottom_port"].x, self.bias_rail_y),
            f"T{i}",
            **t_cross_parameters(
                a=self.a,
                b=self.b,
        ...
        self.insert_cell( WaveguideComposite, nodes=[
                    ...
                    Node(self.refpoints[f"CAP3b_bottom_port"])])
        self.insert_cell( WaveguideComposite, nodes=[
                    Node(self.refpoints[f"T3_port_left"]),
                    Node((self.refpoints[f"CAP2b_bottom_port"].x, self.refpoints[f"T3_port_left"].y)),
                    Node(self.refpoints[f"CAP2b_bottom_port"])])
