...
from kqcircuits.elements.bias_res_cap import BiasResCap
...
parser = argparse.ArgumentParser(description="Run HFSS eigenmode simulations on bias res cap")
...
BaseSimClass = get_single_element_sim_class(BiasResCap)
...
class BiasSpikeResHfssSim(BaseSimClass):
    ...
    def build(self):
        ...
        if not using_lumped:
            ...
            try:
                signal_loc_inductor = self.refpoints['BR_acrl_source_main_inductor']
            except KeyError:
                signal_loc_inductor = self.refpoints['BR_inductor_ground']
