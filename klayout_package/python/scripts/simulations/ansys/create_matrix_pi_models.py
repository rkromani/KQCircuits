...
# This is a Python 2.7 script that should be run in Ansys Electronics Desktop in order to create
# capacitance, inductance, and resistance matrix output variables (for ACRL simulations).
import os
import sys
import time
...
oDesktop.AddMessage("", "", 0, "Creating RLC PI models for all ports (%s)" % time.asctime(time.localtime()))
...
if design_type == "HFSS":
    ...
    # Create capacitance matrix output variables
    oDesktop.AddMessage("", "", 0, "Creating capacitance PI model for all signal nets")
    ...
    # Check if ACRL (inductance/resistance) data is available
    oReportSetup = oDesign.GetModule("ReportSetup")
    try:
        acl_quantities = oReportSetup.GetAllQuantities("Matrix", setup + " : LastAdaptive", ["Context:=", "Original"])
        has_inductance = any("L(" in q for q in acl_quantities)
        has_resistance = any("R(" in q for q in acl_quantities)
    except:
        has_inductance = False
        has_resistance = False

    # Create inductance matrix output variables if ACRL is enabled
    if has_inductance:
        oDesktop.AddMessage("", "", 0, "Creating inductance PI model for all signal nets")
        for i, net_i in enumerate(signal_nets):
            for j, net_j in enumerate(signal_nets):
                if i == j:
                    expression = " + ".join(["L({},{})".format(net_i, net_k) for net_k in signal_nets])
                else:
                    expression = "-L({},{})".format(net_i, net_j)

                oOutputVariable.CreateOutputVariable(
                    "L_{}_{}".format(net_i, net_j),
                    expression,
                    setup + " : LastAdaptive",
                    "Matrix",
                    ["Context:=", "Original"],
    ...
    # Create resistance matrix output variables if ACRL is enabled
    if has_resistance:
        oDesktop.AddMessage("", "", 0, "Creating resistance PI model for all signal nets")
        for i, net_i in enumerate(signal_nets):
            for j, net_j in enumerate(signal_nets):
                if i == j:
                    expression = " + ".join(["R({},{})".format(net_i, net_k) for net_k in signal_nets])
                else:
                    expression = "-R({},{})".format(net_i, net_j)

                oOutputVariable.CreateOutputVariable(
                    "R_{}_{}".format(net_i, net_j),
                    expression,
                    setup + " : LastAdaptive",
                    "Matrix",
                    ["Context:=", "Original"],
...
oDesktop.AddMessage("", "", 0, "RLC PI models created (%s)" % time.asctime(time.localtime()))
